...
<div class="m-4 p-3 width-medium">
    <c:if test="${msg=='success'}"><p style="font-size: x-large; color: blue">Przepis usunięty prawidłowo!</p></c:if>
    <c:if test="${msg=='failed'}"><p style="font-size: x-large; color: red">Coś poszło nie tak podczas usuwania
        przepisu!</p></c:if>
            ...
            <c:forEach items="${recipeList}" var="recipe" varStatus="loopStatus">
                <tr class="d-flex">
                    ...
                    <td class="col-2 d-flex align-items-center justify-content-center flex-wrap">
                        <button onclick="myFunction(${recipe.id},'${recipe.name}')"
...
<script>
    function myFunction(number, name) {
        if (confirm("Czy na pewno chcesz usunąć przepis \"" + name + "\"?")) {
            window.location.href = "/app/deleteRecipe?id=" + number;
        }
    }
</script>
